...
export class DigitalOceanS3Storage extends Storage {
  protected $driver: S3;
  protected $config: DigitalOceanS3StorageConfig;
  protected $bucket: string;

  constructor(config: DigitalOceanS3StorageConfig) {
    super();
   ...
   * This method returns a new instance of DigitalOceanS3Storage.
   */
  public bucket(bucket: string): DigitalOceanS3Storage {
    return new DigitalOceanS3Storage({
      ...this.$config,
      bucket,
    });
  }
  ...
  public async put(
    ...
    options?: S3.Types.PutObjectRequest
  ): Promise<Response> {
    const optParams = options || {};
    const params = { Key: location, Body: content, Bucket: this.$bucket, ...optParams };
    try {
...
export interface DigitalOceanS3StorageConfig extends ClientConfiguration {
  key: string;
  secret: string;
  bucket: string;
}
